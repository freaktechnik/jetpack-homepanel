...
const Section = Class({
    ...
    set data(newData) {
        ...
        HomeProvider.requestSync(this.id, (id) => {
            ...
            dataStorage.save(newData, { replace: true })
                .then((data) => { emit(target, "update", data); return data; },
                      (error) => { throw error; });
        });
    },
    addData: function(newData) {
        ...
        if(!HomeProvider.requestSync(this.id, (id) => {
            ...
            var dataStorage = HomeProvider.getStorage(id);
            dataStorage.save(newData, { replace: false }).then((data) => {
